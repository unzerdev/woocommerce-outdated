...
class UnzerCw_CreditCard extends UnzerCw_PaymentMethod
{
	...
	protected function getMethodSettings(){
		return array(
			'placeholder_size' => array(
 			...
 			'capturing' => array(
				'title' => __("Capturing", 'woocommerce_unzercw'),
 				'default' => 'direct',
 				'description' => __("Should the amount be captured automatically after the order (Direct Charge) or should the amount only be reserved (Authorize)?", 'woocommerce_unzercw'),
 				'cwType' => 'select',
 				'type' => 'select',
 				'options' => array(
					'direct' => __("Direct Charge", 'woocommerce_unzercw'),
 					'deferred' => __("Authorize", 'woocommerce_unzercw'),
 				),
 			),
	...
	public function __construct() {
		$this->icon = apply_filters(
			'woocommerce_unzercw_creditcard_icon', 
			UnzerCw_Util::getResourcesUrl('icons/creditcard.png')
		);
		parent::__construct();
	}
